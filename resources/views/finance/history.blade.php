...
<head>
    ...
    <title>سجل حسابات بنك الراجحي</title>
...
<body>
    ...
    <div class="container mt-4">
        <div class="history-container">
            <!-- رأس الصفحة -->
            <div class="header">
                <h1><i class="fas fa-history"></i> سجل حسابات التمويل</h1>
                   <h1>بنك الراجحي</h1>
                <p class="text-muted">عرض جميع حسابات التمويل الخاصة بك</p>
                <div class="mt-3">
                    <a href="{{ route('finance.calculator') }}" class="btn btn-action btn-calculator">
            ...
            <div class="search-filter">
                <form method="GET" action="{{ route('finance.history') }}" class="row g-3">
                    <div class="col-md-3">
                        ...
                        <select name="status" class="form-select">
                            ...
                            <option value="follow_up" {{ request('status') == 'follow_up' ? 'selected' : '' }}>متابعة
                            </option>
                            <option value="not_sold" {{ request('status') == 'not_sold' ? 'selected' : '' }}>لم يتم
                                البيع</option>
                    ...
                    <table class="table table-custom table-hover">
                        ...
                        <tbody>
                            @foreach ($calculations as $calculation)
                                <tr>
                                    <td class="fw-bold">
                                        {{ $loop->iteration + ($calculations->currentPage() - 1) * $calculations->perPage() }}
                                    </td>
                                    <td>
                                        <div class="fw-bold">{{ $calculation->car_brand ?? 'غير محدد' }}</div>
                                    ...
                                    <td>
                                        <span
                                            class="fw-bold">{{ number_format(floatval($calculation->car_price), 0) }}</span>
                                        <div class="text-muted small">ريال</div>
                                    </td>
                                    <td>
                                        <span
                                            class="fw-bold text-primary">{{ number_format(floatval($calculation->monthly_installment_with_insurance), 0) }}</span>
                                        <div class="text-muted small">ريال/شهر</div>
                                    </td>
                                    <td>{{ $calculation->loan_term_months ?? 0 }} شهر</td>
                                    <td id="status-cell-{{ $calculation->id }}">
                                    ...
                                    <td class="action-buttons">
                                        <a href="{{ route('finance.show', $calculation->id) }}"
                                        ...
                                        <button type="button" class="btn btn-sm btn-warning edit-status-btn"
                                            title="تعديل الحالة" data-id="{{ $calculation->id }}"
                                            data-current-status="{{ $calculation->status }}"
                                            data-car-brand="{{ $calculation->car_brand ?? 'غير محدد' }}"
                                            data-car-model="{{ $calculation->carModel->model_year ?? 'غير محدد' }}">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <form action="{{ route('finance.delete', $calculation->id) }}" method="POST"
                                            class="d-inline"
                ...
                <div class="row mt-4">
                    <div class="col-md-12">
                        <div class="alert alert-info">
                            ...
                            إجمالي النتائج: <strong>{{ $calculations->total() }}</strong> حساب |
                            الصفحة {{ $calculations->currentPage() }} من {{ $calculations->lastPage() }}
                        </div>
                    </div>
                </div>
            @else
                <div class="no-data">
                    ...
                    <h4>لا توجد حسابات حتى الآن</h4>
                    <p class="text-muted">ابدأ بحساب جديد لرؤية سجل الحسابات هنا</p>
                    <a href="{{ route('finance.calculator') }}" class="btn btn-action btn-calculator mt-3">
                        <i class="fas fa-calculator"></i> إنشاء حساب جديد
                    </a>
                </div>
            @endif
        </div>
                ...
                <form id="updateStatusForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <p class="mb-0"><strong>الحالة الحالية:</strong> <span id="modalCurrentStatus"></span>
                            </p>
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    document.getElementById('modalCarInfo').textContent =
                        `${carBrand} (${carModel})`;
                    document.getElementById('modalCurrentStatus').textContent = statusLabels[
                        currentStatus];
            ...
            updateForm.addEventListener('submit', async function(e) {
                ...
                try {
                    const response = await fetch(`/finance/${calculationId}/update-status`, {
                        method: 'POST',
                        headers: {
