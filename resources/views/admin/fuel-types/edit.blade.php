...
@section('title', 'تعديل نوع الوقود')

@section('page-title', 'تعديل نوع الوقود')
        ...
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="card-title mb-0 text-white">
                    <i class="fas fa-edit me-2"></i>تعديل نوع الوقود: {{ $type->name }}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.fuel-types.update', $type->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" 
                               ...
                               placeholder="أدخل اسم نوع الوقود الجديد"
                               required
                               autofocus>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="text-muted">يمكنك تعديل اسم نوع الوقود، ولكن تأكد من أن الاسم الجديد فريد وغير مكرر.</small>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        <div>
                            <a href="{{ route('admin.fuel-types.show', $type->id) }}" class="btn btn-secondary me-2">
                                <i class="fas fa-eye me-1"></i> عرض
                            </a>
                            <a href="{{ route('admin.fuel-types.index') }}" class="btn btn-outline-secondary">
